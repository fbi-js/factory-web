...
const getConfig = (env) => {
    ...
    function baseConfigDefault({ orgName = 'project-name', projectName = 'root-config', opts: any = opts, webpackConfigEnv = {
        isLocal: "true",
        COS_ENV: 'development'
    }, publicPath = '', }) {
    ...
    function getDevAppConfig(mode) {
        ...
        if (mode === 'runByChildApp') {
            const { selfApp, otherApps } = getAppConfig(path_1.default.resolve('../../../apps.config'));
